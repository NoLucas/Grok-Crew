...
export default function ConnectConsole() {
  ...
  return <><SiteHeader current="connect" />
    <main className="connect-main"><section className="connect-hero"><div><p className="kicker"><LocalizedText ko="로컬 Grok 전달 도구" en="LOCAL GROK HANDOFF DESK" /></p><h1><LocalizedText ko="모든 편집을" en="Keep every edit" /><br /><span><LocalizedText ko="내 컴퓨터 안에 두세요." en="on your machine." /></span></h1><p><LocalizedText ko="Reel Forge는 외부 API 키나 클라우드 연결 없이 로컬에서 실행됩니다. Grok은 수동 브리프 전달로 참여하거나, 같은 PC에서 실행되는 bot이라면 Local Studio에 입장해 계획 작업을 시작할 수 있습니다." en="Reel Forge runs locally with no external API key or cloud connection. Grok can participate by manual brief handoff, or a bot running on this same PC can enter Local Studio and begin planning work." /></p></div><aside className="bridge-status ready"><span><LocalizedText ko="로컬 모드" en="LOCAL MODE" /></span><b>LOCAL ONLY</b><p><LocalizedText ko="외부 API 없음 · 키 없음 · 같은 PC의 입장만 허용" en="No external API · no key · same-PC entry only" /></p><button onClick={refresh}><LocalizedText ko="저장된 데이터 새로고침" en="Refresh saved data" /></button></aside></section>
      ...
      <section className="connect-grid bridge-overview"><article className="connect-card local-address"><div className="connect-card-head"><span>01 · LOCAL RUN</span><em>One device</em></div><b>localhost · browser storage · Local Studio</b><p>Start Reel Forge on your own computer without a cloud account or API key. A bot can use the optional Local Studio entry only from this same PC.</p><div><button onClick={() => void copy('manifest', JSON.stringify(manifest, null, 2))}>{copied === 'manifest' ? 'Brief copied' : 'Copy current brief'}</button><button onClick={() => download('noh-offline-handoff.json', manifest)}>Download JSON</button></div></article><article className="connect-card setup-card"><div className="connect-card-head"><span>02 · LOCAL-ONLY RULE</span><em>Safe by default</em></div><ol><li>No xAI key is requested or stored.</li><li>No background request is sent to a remote Grok service.</li><li>Optional bot entry binds only to 127.0.0.1 on this PC.</li><li>No post can be shared or published here.</li></ol><p>Use the exported JSON manually, or run a same-PC bot through the Local Studio entry contract.</p></article></section>
      ...
      <section className="connect-rules"><p className="kicker">USAGE RULE</p><h2>Hand Grok the brief.<br />Keep the work local.</h2><div><span>✓ Local project state</span><span>✓ Local gate checks</span><span>✓ Same-PC bot entry</span><span>× External API key</span><span>× Automatic publishing</span><span>× Remote bot access</span></div></section>
