...
export default function ConnectConsole() {
  ...
  return <><SiteHeader current="connect" />
    <main className="connect-main"><section className="connect-hero"><div><p className="kicker"><LocalizedText ko="오프라인 Grok 전달 도구" en="OFFLINE GROK HANDOFF DESK" /></p><h1><LocalizedText ko="모든 편집을" en="Keep every edit" /><br /><span><LocalizedText ko="내 컴퓨터 안에 두세요." en="on your machine." /></span></h1><p><LocalizedText ko="Reel Forge는 API 키, 외부 제공자 연결, 자동 데이터 전송 없이 로컬에서 실행됩니다. Grok은 사용자가 브리프를 복사해 전달하고 계획을 다시 붙여 넣을 때만 참여합니다." en="Reel Forge runs locally with no API key, no provider connection, and no automated data transfer. Grok participates only when you choose to copy a brief out and paste a plan back in." /></p></div><aside className="bridge-status ready"><span><LocalizedText ko="로컬 모드" en="LOCAL MODE" /></span><b>OFFLINE</b><p><LocalizedText ko="API 없음 · 키 없음 · 네트워크 요청 없음" en="No API · no key · no network request" /></p><button onClick={refresh}><LocalizedText ko="저장된 데이터 새로고침" en="Refresh saved data" /></button></aside></section>
