...
function createLocalManifest(): Json {
  const agent = readLocal('nohAgentDesk'); const edit = readLocal('nohEditLab'); const project = readLocal('currentProject'); const captionPacket = readLocal('packet');
  return {
    schema: 'noh.reel-forge.offline-handoff/v1', generatedAt: new Date().toISOString(), mode: 'offline_local_only', project,
    ...
    workflow: { grok: 'Copy this file to Grok manually. Paste the returned plan back into this local desk for review.', allowed: ['read_brief', 'propose_edit_plan', 'identify_gate_risks'], prohibited: ['automatic_publish', 'automatic_share', 'source_asset_reuse', 'review_bypass'] },
  };
}
...
export default function ConnectConsole() {
  ...
  useEffect(() => { const hydrate = window.setTimeout(() => { setManifest(createLocalManifest()); setBotReturn(window.localStorage.getItem('nohOfflineGrokReturn') ?? ''); }, 0); return () => window.clearTimeout(hydrate); }, []);
  ...
  const saveReturn = () => { window.localStorage.setItem('nohOfflineGrokReturn', botReturn); setSaved(true); window.setTimeout(() => setSaved(false), 1700); };
  const workflow = [['01', 'Prepare locally', 'Edit the Studio, Agent Desk, and Edit Lab. Nothing leaves this browser.'], ['02', 'Export the brief', 'Copy or download one JSON handoff from this desk.'], ['03', 'Ask Grok manually', 'Paste the brief into any Grok conversation you choose.'], ['04', 'Bring back the plan', 'Paste Grok’s response below and keep it saved only on this device.']];
  return <><header className="forge-header connect-header"><a href="/" className="wordmark"><span>NOH</span><i>Reel Forge</i></a><nav aria-label="Primary navigation"><a href="/">Studio</a><a href="/edit">Edit lab</a><a href="/cut">Cut log</a><a href="/library">Library</a><a href="/agent">Agent desk</a><a className="current" href="/connect">Local desk</a><a href="/packet">Packet</a><a href="/gates">Gate board</a><a href="/export">Export</a></nav><LanguageSwitcher /><div className="header-meta"><span>Offline workspace · this device only</span><b>Created with Grok</b></div></header>
    <main className="connect-main"><section className="connect-hero"><div><p className="kicker">OFFLINE GROK HANDOFF DESK</p><h1>Keep every edit<br /><span>on your machine.</span></h1><p>Reel Forge runs locally with no API key, no provider connection, and no automated data transfer. Grok participates only when you choose to copy a brief out and paste a plan back in.</p></div><aside className="bridge-status ready"><span>LOCAL MODE</span><b>OFFLINE</b><p>No API · no key · no network request</p><button onClick={refresh}>Refresh saved data</button></aside></section>
      ...
      <section className="connect-grid bridge-overview"><article className="connect-card local-address"><div className="connect-card-head"><span>01 · LOCAL RUN</span><em>One device</em></div><b>localhost · browser storage</b><p>Start Reel Forge on your own computer and work without a cloud account, API key, or bot endpoint.</p><div><button onClick={() => void copy('manifest', JSON.stringify(manifest, null, 2))}>{copied === 'manifest' ? 'Brief copied' : 'Copy current brief'}</button><button onClick={() => download('noh-offline-handoff.json', manifest)}>Download JSON</button></div></article><article className="connect-card setup-card"><div className="connect-card-head"><span>02 · NO-CONNECTION RULE</span><em>Deliberately manual</em></div><ol><li>No xAI key is requested or stored.</li><li>No background request is sent to Grok.</li><li>No server API is required for gate checks.</li><li>No post can be shared or published here.</li></ol><p>Use the exported JSON only when you personally decide to give Grok context.</p></article></section>
      <section className="connect-card endpoint-card"><div className="connect-card-head"><span>03 · OFFLINE WORKFLOW</span><em>Copy → think → paste</em></div><div className="endpoint-list offline-list">{workflow.map(([number, name, detail]) => <article key={number}><span>{number}</span><div><b>{name}</b><p>{detail}</p></div><code>LOCAL</code></article>)}</div></section>
      <section className="connect-grid action-grid"><article className="connect-card manifest-card-local"><div className="connect-card-head"><span>04 · CURRENT OFFLINE BRIEF</span><em>Browser → JSON file</em></div><p>One portable snapshot of your saved Studio, Agent Desk, Edit Lab, and caption packet.</p><div className="manifest-actions"><button onClick={refresh}>Refresh saved data</button><button onClick={() => void copy('manifest-panel', JSON.stringify(manifest, null, 2))}>{copied === 'manifest-panel' ? 'Brief copied' : 'Copy JSON brief'}</button><button onClick={() => download('noh-offline-handoff.json', manifest)}>Download JSON</button><button onClick={runValidation}>Run offline gate check</button></div><pre>{JSON.stringify(manifest, null, 2)}</pre></article><article className="connect-card plan-card local-return"><div className="connect-card-head"><span>05 · GROK RETURN INBOX</span><em>Paste manually</em></div><label>Paste Grok’s edit plan<textarea rows={11} value={botReturn} onChange={(event) => setBotReturn(event.target.value)} placeholder="Paste a Grok answer here after you voluntarily share the exported brief." /></label><button className="plan-button" onClick={saveReturn}>{saved ? 'Saved on this device' : 'Save return locally'}</button><p>This text remains in this browser’s local storage. It is not sent anywhere by Reel Forge.</p></article></section>
      ...
      <section className="connect-rules"><p className="kicker">USAGE RULE</p><h2>Hand Grok the brief.<br />Keep the work local.</h2><div><span>✓ Local project state</span><span>✓ Local gate checks</span><span>✓ Manual Grok handoff</span><span>× API key</span><span>× Automatic publishing</span><span>× Network bot access</span></div></section>
    </main><footer className="forge-footer"><span>NOH | AI at work</span><span>Offline handoff · local storage only</span><span>Created with Grok</span></footer></>;
}
