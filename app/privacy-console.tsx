    ...
    <>
      ...
      <main className="privacy-main">
        <section className="privacy-hero">
          <p className="kicker">{t("로컬 개인정보 보호 + 맞춤 설정", "LOCAL PRIVACY + PERSONALIZATION")}</p>
          <h1>
            {t("이름은 내가 정하고,", "Name the workspace yourself,")}
            <br />
            <span>
              {t(
                "기록은 내 기기에만 둡니다.",
        ...
        <section className="privacy-grid">
          <article className="privacy-card">
            <div className="privacy-card-head">
              <span>{t("작업 공간 이름", "WORKSPACE IDENTITY")}</span>
              <em>{t("이 기기 전용", "this device only")}</em>
            </div>
            <label>
              {t("작업 공간 이름", "Workspace name")}
              <input
                value={workspaceName}
                onChange={(event) => setWorkspaceName(event.target.value)}
                maxLength={80}
                placeholder={t("예: 내 영상 작업실", "e.g. My video workspace")}
              />
            </label>
            <label>
              {t("기본 봇 표시명", "Default bot label")}
              <input
                value={botLabel}
                onChange={(event) => setBotLabel(event.target.value)}
                maxLength={80}
                placeholder={t("예: 로컬 편집 봇", "e.g. Local editor bot")}
              />
            </label>
            <button onClick={saveProfile}>
              {t("이 기기에만 저장", "Save on this device")}
            </button>
            <p>
              {t(
          ...
          <article className="privacy-card privacy-boundary">
            <div className="privacy-card-head">
              <span>{t("데이터 범위", "DATA BOUNDARY")}</span>
              <em>{t("이 기기 전용", "LOCAL ONLY")}</em>
            </div>
            <ul>
              <li>
                {t(
