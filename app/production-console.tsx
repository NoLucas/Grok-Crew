...
export default function ProductionConsole() {
  ...
  const contract = JSON.stringify({ scope: '127.0.0.1 only', actor: 'Grok bot', on_entry: 'all local editing features enabled; local render defaults to auto_local', bot_choice: ['auto_local render', 'approval_required render'], always_human: ['Instagram queue approval', 'Instagram publish switch', 'PUBLISH confirmation'], never: ['read Meta credentials', 'access outside workspace'] }, null, 2);
  ...
  return <>
    ...
    <main className="production-main">
      ...
      <div className="production-note"><b>LOCAL FIRST</b><span>소스는 <code>local_studio/workspace/inputs</code>, 결과물은 <code>workspace/outputs</code>, 프로젝트·작업 이력은 SQLite에 저장됩니다. 입장한 봇은 모든 로컬 기능을 사용할 수 있고, 로컬 렌더는 기본 자동 실행 또는 사람 승인 모드를 봇별로 선택합니다.</span></div>
