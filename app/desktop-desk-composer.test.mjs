...
describe('desk composer restage', () => {
  it('Connect shows one family at a time and folds extras', () => {
    ...
    assert.match(text, /GROK_CREW_OK만 보내고 멈추면 아직 일이 안 간/);
    assert.equal(text.includes('봇이 보낸 한 줄'), false);
    assert.equal(text.includes('desktop-bot-confirm'), false);
    assert.equal(text.includes('그 글을 복사하면 이 책상이 그 자리로 입장합니다'), false);
    assert.match(text, /markRemoteCopied/);
    assert.match(text, /isBareConnectReply/);
    assert.match(text, /confirmRemoteReplies/);
