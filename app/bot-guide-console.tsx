...
const fallbackGuide: Guide = {
  schema: 'noh.reel-forge.bot-guide/v1', title: 'NOH Reel Forge Local Editing Playbook', scope: 'Same workstation only.', first_action: 'Enter Local Studio to record the first heartbeat before preparing work.',
  ...
  workflow: [{ step: 1, name: 'enter_local_studio', action: 'POST /api/bot-entry', goal: 'Record a local entry and first heartbeat.' }, { step: 2, name: 'read_the_guide', action: 'GET /api/bot-guide', goal: 'Load the local rules before planning.' }, { step: 3, name: 'set_edit_method', action: 'POST /api/edit-method', goal: 'Set only the permitted local edit controls.' }, { step: 4, name: 'plan_the_cut', action: 'Create transcript-first EDL', goal: 'Keep the hook and remove filler.' }, { step: 5, name: 'prepare_finish', action: 'Set reframe, sound, look, captions, quality, and FPS', goal: 'Make the smallest useful changes.' }, { step: 6, name: 'request_review', action: 'Record await_human_approval', goal: 'Stop before output actions.' }, { step: 7, name: 'render_after_approval', action: 'Queue and run an approved render', goal: 'Report the MP4 path or exact failure.' }, { step: 8, name: 'publish_after_approval', action: 'Queue an approved Instagram job', goal: 'Require a separate publish confirmation.' }],
