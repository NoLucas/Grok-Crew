...
## 자동 스위치
자막·더빙은 운영자가 자동에서 켠 뒤에만 계획에 적습니다. 초대문에 「자막 끔」이면 음성인식·자막을 시키지 않습니다. 「더빙 끔」이면 소리를 바꾸지 않습니다.
`;
...
## 자동 스위치
초대문에 「자막 끔」이면 음성인식·자막을 하지 않습니다. 「자막 켬」일 때만 말 구간을 자막·word_timings로 붙입니다. 「더빙 끔」이면 원본 소리를 유지합니다. 더빙은 운영자가 넣은 음성만, TTS는 만들지 않습니다.
...
export function voiceInviteBlock(language = 'ko', voice: VoiceInvite = {}): string {
  ...
  const dubbing = Boolean(voice.dubbing);
  const lang = language.slice(0, 2);
  if (lang === 'zh') {
    return captions
      ? [
          '字幕：开。只识别说话的段落，再写成字幕。源语言和去向不同时只改字幕，声音仍是原片。',
          dubbing ? '配音：开。只用操作员放进的语音。没有就不改声音。不要做 TTS。' : '配音：关。不要配音，不要改原声。',
        ].join('\n')
      : [
          '字幕：关。不要语音识别，不要烧字幕。',
          dubbing ? '配音：开。只用操作员放进的语音。没有就不改声音。不要做 TTS。' : '配音：关。不要配音，不要改原声。',
  ...
  if (lang === 'ja') {
    return captions
      ? [
          '字幕：オン。話している区間だけ認識して字幕にする。元の言語と送り先が違うときは字幕だけ変える。音は元のまま。',
          dubbing ? '吹き替え：オン。運営者が入れた音声だけ使う。なければ元の音。TTS は作らない。' : '吹き替え：オフ。吹き替えしない。元の音を変えない。',
        ].join('\n')
      : [
          '字幕：オフ。音声認識も字幕焼き込みもしない。',
          dubbing ? '吹き替え：オン。運営者が入れた音声だけ使う。なければ元の音。TTS は作らない。' : '吹き替え：オフ。吹き替えしない。元の音を変えない。',
        ].join('\n');
  }
  if (lang === 'en') {
    return captions
      ? [
          ...
          dubbing ? 'Dubbing: on. Use only the operator’s audio files. If none, keep the original. Do not make TTS.' : 'Dubbing: off. Do not dub. Do not replace the original audio.',
        ].join('\n')
      : [
  ...
  return captions
    ? [
        ...
        dubbing ? '더빙: 켬. 운영자가 넣은 음성 파일만 씁니다. 없으면 원본 소리. TTS를 만들지 않습니다.' : '더빙: 끔. 더빙하지 않습니다. 원본 소리를 바꾸지 않습니다.',
      ].join('\n')
    : [
