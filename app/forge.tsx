...
function sceneScale(scene: Scene, progress: number) {
...
function UiText({ ko, en }: { ko: string; en: string }) {
  const { t } = useLanguage();
  return <>{t(ko, en)}</>;
}
...
function useForge() {
  ...
  const [packet, setPacket] = useState<Packet>(initialPacket);
  ...
  useEffect(() => {
    ...
    setProject(saved?.sceneA && saved?.sceneB ? saved : blankProject);
    setPacket(safeRead<Packet>("localVideoPacket", initialPacket));
    setLastTemplates(safeRead<string[]>("localVideoTemplates", []));
    setLastHooks(
      safeRead<{ hook: string; at: number }[]>("localVideoHooks", []),
    );
    setSlots(safeRead<Record<string, number>>("localVideoSlots", {}));
    setReady(true);
  }, []);
  useEffect(() => {
    if (ready) {
  ...
  }, [project, packet, lastTemplates, lastHooks, slots, ready]);
  ...
  const hookBlocked = lastHooks.some(
    (entry) =>
      ...
      Date.now() - entry.at < 8 * 60 * 60 * 1000,
  );
...
function ExportRoom({ forge }: { forge: ReturnType<typeof useForge> }) {
