...
const workspaces = [
  { href: '/production', index: '01', ko: '프로젝트 만들기', en: 'Create a project', koDetail: '원본 경로와 편집 구간을 넣고, 승인된 로컬 렌더 대기열을 준비합니다.', enDetail: 'Add source paths and edit segments, then prepare an approved local render queue.' },
...
const steps = [
  ...
  { ko: '사람 승인 뒤 렌더', en: 'Render after human approval', detailKo: '봇은 계획과 검사를 진행하고, 렌더와 실제 게시에는 사람이 마지막 결정을 합니다.', detailEn: 'Bots plan and inspect; a person makes the final decision for rendering and publishing.' },
];
...
export default function HomeConsole() {
  ...
  return <><SiteHeader current="studio" /><main className="home-main">
    ...
    <section className="home-safety"><b>{t('봇이 할 수 있는 일', 'What bots can do')}</b><p>{t('프로젝트·컷 맵·검사·작업 보드·편집 방식을 준비할 수 있습니다. 렌더와 Instagram 게시에는 여전히 사람 승인이 필요합니다.', 'Bots can prepare projects, cut maps, checks, task boards, and edit methods. Rendering and Instagram publishing still require human approval.')}</p><Link href="/bot-guide">{t('봇 작업 설명서 보기', 'Read the bot guide')} →</Link></section>
  </main></>;
}
