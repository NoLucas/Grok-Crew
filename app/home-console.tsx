    ...
    <>
      ...
      <main className="home-main">
        <section className="home-hero">
          <div>
            ...
            <h1>
              ...
              <br />
              <span>
                {t("내 컴퓨터의 영상 제작실.", "bots on your own computer.")}
              </span>
            </h1>
            <p>
              {t(
            ...
            <div className="home-hero-actions">
              <Link href="/production">
                {t("첫 프로젝트 만들기", "Create your first project")} →
              </Link>
              <Link href="/terminal" className="home-secondary-action">
                {t("봇 터미널 열기", "Open bot terminal")}
              </Link>
            </div>
          </div>
          <aside>
            ...
            <div>
              <i>01</i>
              <strong>{t("파일", "Files")}</strong>
              <em>{t("이 기기", "This device")}</em>
            </div>
            <div>
              <i>02</i>
              <strong>{t("봇 기록", "Bot records")}</strong>
              <em>SQLite</em>
            </div>
            <div>
              <i>03</i>
              <strong>{t("최종 승인", "Final approval")}</strong>
              <em>{t("사람", "Human")}</em>
            </div>
          </aside>
        </section>
          ...
          <div className="home-step-grid">
            {steps.map((step, index) => (
              <article key={step.en}>
                ...
                <div>
                  <b>{t(step.ko, step.en)}</b>
                  <p>{t(step.detailKo, step.detailEn)}</p>
                </div>
              </article>
            ))}
          </div>
          ...
          <div className="home-workspace-grid">
            {workspaces.map((space) => (
              <Link href={space.href} key={space.href}>
                <i>{space.index}</i>
                <b>{t(space.ko, space.en)}</b>
                <p>{t(space.koDetail, space.enDetail)}</p>
                <span>{t("열기", "Open")} →</span>
              </Link>
            ))}
          </div>
        ...
        <section className="home-safety">
          ...
          <Link href="/bot-guide">
            {t("봇 작업 설명서 보기", "Read the bot guide")} →
          </Link>
        </section>
      </main>
    </>
