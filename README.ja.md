...
3. **つなぐ** — 上の **ボット** メニューで、この PC か別 PC（Grok・自分のエージェント）を選ぶ。この PC ならチェックイン文を貼る。別 PC なら接続文を貼り、ボットが返した `GROK_CREW_OK` の一行を戻す。名前が出ればつながった。
4. **書く** — タイトル。
...
最初の画面は **ボット** メニューです。この PC も別 PC もそこでつなぎます。ボットが付くか **自分で開く** で映像を開くと、設定・編集・書き出しが付きます。そのあと短い机で **任せる**はタイトル → コピー → 待ち。**自分で開く**は映像を置く → すぐ編集。二つの道は一枚のカードに混ぜません。サンプルは **サンプルで画面を見る** だけで開きます。コピーのあとは、ボット作業中・最後の確認・まだない / 到着 / 失敗を人の言葉で出します。python・ポート・フォルダ経路は開くまで隠しです。**もっと詳しく**と**高度なツール**は初回カットが届いてから大きくなります。編集ドアに届いたカットはボタンなしで開きます。ボットが置いたフォルダは1行に畳まれ、開くと左が一覧・右がプレビューです。右クリックでプレビュー・拡大・原本表示・削除を選べます。画質は仕様ロック、画面比・字幕と残りの編集操作はデスクで変えられます。設定でショート・リール・TikTok などのスタイルを選ぶか、今の値を名前で保存できます。明るい昼・暗い夜・やわらかい昼・やわらかい夜と文字は左上の歯車で変えます。このデスクはサイトを掻きません。

そのあとは 4 つのタブです。

| タブ | 用途 |
| --- | --- |
| **ボット** | 接続状態、この PC のチェックイン、別 PC のコード |
...
別のパソコンの AI はこの机を開けません。**ボット** メニューは名前だけつなぎ、終わったカットはこの窓に置きます。よそにあるボットに「ちょっと見せて」と映像を送りません。
