...
最初の画面は短いです。タイトル、映像を置く場所、**ボットにこの文をコピー**。その保存はボット一人です（`source_mode: bot`）。同じ PC の Cursor エージェントはチェックインでき、別の PC の Cursor は招待文の編集受信箱フォルダにだけ置きます。スタイルレシピと二人クルーは **もっと詳しく** の後ろです。編集ドアに届いたカットはボタンなしで開きます。ボットが置いたフォルダは1行に畳まれ、開くと左が一覧・右がプレビューです。右クリックでプレビュー・拡大・原本表示・削除を選べます。画質は仕様ロック、画面比・字幕と残りの編集操作はデスクで変えられます。設定でショート・リール・TikTok などのスタイルを選ぶか、今の値を名前で保存できます。明るい昼・暗い夜・やわらかい昼・やわらかい夜と文字は左上の歯車で変えます。このデスクはサイトを掻きません。
