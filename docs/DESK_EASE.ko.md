...
| 화면 | 위계 | 상태 |
| --- | --- | --- |
| 종료 대화 | 제목 “숨기기와 종료는 다릅니다”. 위 카드는 숨기기(호박 줄), 아래 카드는 종료(빨강 줄). 버튼은 종료(빨강) · 돌아가기. | 브라우저에서는 이 카드. 패키지 창의 네이티브 경고는 같은 문장. |
| 어제랑 같게 | 연결 탭, 자리 목록 **위**. 한 줄 요약(나라 · 올릴 곳 · 세 자리) + 가로로 꽉 찬 기본 버튼. | 복사함 / 막히면 textarea. “세 봇이 받았다” 금지. |
| 대기 다시 복사 | “봇 창에 붙이세요” 바로 아래. 버튼이 제목 다음으로 크다. | 빈 초대문이면 꺼짐. 막히면 기존 textarea. |
| 크루 보드 | 자동 대기·도착과 **같은 컴포넌트**를 연결에도. 파이프 세 칸, 그다음 스레드. 메모 버튼은 스레드 제목 오른쪽. | 빈 자리 / 로딩 / 오류 / 다음 자리 연결되지않음(호박) / 자리 확인 N분 끊김. 읽음 없음. |
