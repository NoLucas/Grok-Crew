...
빈 화면의 기본은 **연결** 메뉴입니다. 다른 PC(Grok·Cursor·Claude·내가 만든 에이전트)가 맨 위이고, 이 PC 체크인·Local Studio·GitHub·Runner도 거기서 합니다. 초록불이 **연결됨**입니다. 봇이 붙거나 **내가 열기**로 영상을 열면 설정·편집·내보내기가 켜집니다. 그다음 짧은 책상에서 **맡기기**는 제목 → 복사 → 기다림. **내가 열기**는 영상 놓기 → 바로 편집. 샘플은 **샘플로 화면 보기**로만 엽니다. 복사 글은 클립보드에만 가고, 화면에는 붙이라는 말과 대기만 둡니다. 레시피·두 봇·고급 도구는 첫 컷이 온 뒤 **더 자세히**에만 있습니다. 연결 확인은 `docs/CURSOR_AGENT.ko.md`.
