...
빈 화면의 기본은 짧은 책상입니다. **맡기기**는 제목 → 복사 → 기다림. **내가 열기**는 영상 놓기 → 바로 편집. 샘플은 **샘플로 화면 보기**로만 엽니다. 복사 글은 클립보드에만 가고, 화면에는 붙이라는 말과 대기만 둡니다. 레시피·두 봇·고급 도구는 첫 컷이 온 뒤 **더 자세히**에만 있습니다. 연결 확인은 `docs/CURSOR_AGENT.ko.md`.
