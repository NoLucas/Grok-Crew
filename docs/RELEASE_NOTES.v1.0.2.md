...
공개 파일: **Windows에서 열기** → `GrokCrew-Windows.exe`. 받는 곳: 홈페이지에서 인증한 뒤 열리는 Google 드라이브 파일. 이미 설치한 책상은 [v1.0.2 업데이트](https://github.com/NoLucas/Grok-crew-test/releases/tag/v1.0.2)를 봅니다.

1.0.1과 같은 길입니다. 받아서 엽니다. 계정 없고, 카드 없고, 크레딧 없습니다.

영상을 대신 만들어 주는 구독 서비스가 아닙니다. 쓰던 Grok Bot이나 Agent를 붙이면, 다듬은 파일이 **이 PC 폴더**에 남습니다.

## 1.0.1에서 달라진 것

- 처음 열면 이 PC에 받을 TTS를 고릅니다. 그 창이 이제 책상 전체를 씁니다. 예전에는 옆칸 너비에 찌그러져 보였습니다.
- 설치는 그대로 한 번 클릭입니다. 고르지 않으면 Kokoro-82M입니다. 자동에서 TTS를 켜기 전에는 쓰지 않습니다.

## 받아서 연다

1. `GrokCrew-Windows.exe`를 받아 엽니다. 관리자 비밀번호를 묻지 않습니다. 서명이 없어 파란 보호 화면이면 **추가 정보 → 그래도 실행**.
2. 처음 열면 어떤 TTS를 받을지 고릅니다. 고른 뒤에 책상이 열립니다.
3. **연결**에서 쓰던 봇을 붙입니다. 붙일 글을 봇 창에 넣습니다.
4. **자동**에 하고 싶은 말을 적고 **만들기**를 누릅니다.
5. 사람이 봇 창에 붙여 넣습니다. 이 창은 기다립니다.
6. 미리보기가 뜨면 **이 PC에 저장**합니다.
