...
공개 파일: **Windows에서 열기** → `GrokCrew-Windows.exe`. 받는 곳: 홈페이지에서 인증한 뒤 열리는 Google 드라이브 파일. 이미 설치한 책상은 [v1.0.3 업데이트](https://github.com/NoLucas/Grok-crew-test/releases/tag/v1.0.3)를 봅니다.

1.0.2와 같은 길입니다. 받아서 엽니다. 계정 없고, 카드 없고, 크레딧 없습니다.

영상을 대신 만들어 주는 구독 서비스가 아닙니다. 쓰던 Grok Bot이나 Agent를 붙이면, 다듬은 파일이 **이 PC 폴더**에 남습니다.

## 1.0.2에서 달라진 것

- **연결**에서 Grok Bot 글을 복사해 봇 창에 붙입니다. 봇은 등록된 Windows에서 승인 후 이 책상에 체크인합니다. 그 자리 램프가 **연결됨**이 됩니다.
- 봇 쪽 Linux에서 `127.0.0.1`을 열면 이 창이 아닙니다. 체크인이 안 되면 봇이 보낸 `GROK_CREW_OK` 한 줄을 연결 칸에 붙입니다. 복사만으로는 초록불이 켜지지 않습니다.
- 설치는 그대로 한 번 클릭입니다. 처음 열면 TTS를 고릅니다. 고르지 않으면 Kokoro-82M입니다.
