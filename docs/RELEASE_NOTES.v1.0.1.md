...
공개 파일: **Windows에서 열기** → `GrokCrew-Windows.exe`. 받는 곳: [v1.0.1 릴리스](https://github.com/NoLucas/Grok-Crew/releases/tag/v1.0.1).

1.0.0과 같은 길입니다. 받아서 엽니다. 계정 없고, 카드 없고, 크레딧 없습니다.

영상을 대신 만들어 주는 구독 서비스가 아닙니다. 쓰던 Grok Bot이나 Agent를 붙이면, 다듬은 파일이 **이 PC 폴더**에 남습니다.

## 1.0.0에서 달라진 것

- 처음 열면 이 PC에 받을 TTS를 고릅니다. 설치는 그대로 한 번 클릭입니다. 고르지 않으면 Kokoro-82M입니다. 자동에서 TTS를 켜기 전에는 쓰지 않습니다.
- 연결 램프는 **연결됨** / **연결되지않음**만 씁니다. 붙일 글을 복사해도 초록불이 켜지지 않습니다.
