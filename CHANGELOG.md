...
- Auto captions, dubbing, and **TTS** stay **off** until the operator turns them on. Captions on runs VAD then whisper.cpp. Dubbing on uses operator audio only; if none, keep the original. TTS on uses one chosen model (Kokoro-82M if they only press Next; Step Audio EditX or Zonos-v0.1 have hardware warnings). TTS off never generates a voice. First open does not block on the voice picker; the three cards appear when TTS is on (Auto or Setup). Additive `GET /api/v2/first-run` `voice_model` and `POST /api/v2/first-run/voice-model`.
