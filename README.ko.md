...
**Grok Crew는 내 컴퓨터의 숏폼 화면입니다. 규격은 사람이 적고, 문은 두 개입니다. 편집 문과 수집·다른 에이전트 문.**
...
```
내 규격  →  수집 보낼함 + 편집 보낼함  →  수집 봇이 자료함에 클립을 둠  →  편집 봇이 그 클립만 자름  →  편집 인박스  →  이 PC가 받음  →  내가 원할 때만 게시
```
...
첫 화면은 짧습니다. 제목, 영상 놓기, **봇에게 이 말 복사**. 그 저장은 봇 하나입니다(`source_mode: bot`). Cursor 에이전트는 같은 PC면 체크인하고, 다른 컴퓨터면 초대문의 편집 인박스 폴더에만 둡니다. 스타일 레시피와 봇 둘 규격은 **더 자세히** 뒤에 있습니다. 편집 문으로 온 컷은 받기 버튼 없이 열립니다. 봇이 저장한 폴더(`inputs/handoff/` 패키지와 자료함 클립)는 한 줄로 접혀 있고, 펼치면 목록 옆에 미리보기가 있습니다. 파일을 오른쪽 클릭하면 미리보기·크게 보기·원본 보기·삭제를 고릅니다. 화질은 규격 잠금이고, 화면 비율·자막과 나머지 편집 조절은 설정에서 바꿀 수 있습니다. 설정에서 유튜브 쇼츠·인스타 릴스·틱톡·사용자 지정을 고른 뒤 지정하고, 지금 값은 따로 이름으로 저장할 수 있습니다. 프로젝트는 폴더로 묶고, 이름 바꾸기와 휴지통(복원·30일 후 삭제)을 쓸 수 있습니다. 밝은 낮·어두운 밤·부드러운 낮·부드러운 밤과 글자 강조·크기는 왼쪽 위 톱니바퀴에서 바꿉니다. 예전 콘솔은 **고급 도구**가 새 탭으로 엽니다. 사람은 봇이 쓸 도구를 지정할 수 있고, 실행은 웬만하면 봇이 적힌 API로 합니다. 허브는 카드만 두고 카탈로그 JSON은 접으며, 테마는 이 설정을 따릅니다. 이 앱은 사이트를 긁지 않습니다.
