...
- [x] 자막용 오픈라이선스 폰트 번들링 (시스템 폰트를 찾지 못해 렌더가 실패하는 문제 방지)
- [x] 렌더 동시 실행 개수를 조용한 기본값 1이 아니라 문서화된 설정으로 노출
- [x] 로컬 CORS/Origin 허용 목록을 포트 3000 고정이 아니라 설정 가능하게
- [x] 렌더 파이프라인과 앱을 위한 자동화 테스트 스위트
- [x] 평평한 볼륨 조절이 아니라 대사 구간에서 자동으로 음악을 줄이는 덕킹
- [x] PR마다 린트·빌드·Python 검사를 실행하는 GitHub Actions CI
- [x] 클라우드 봇 인계 채널 보안 강화 (미디어 크기 제한, 번들 크기 제한, 주기당 최대 처리 개수)
- [x] README 번역에 맞춰 실제 앱 UI와 봇 설명서에 중국어·일본어 지원 확장
- [ ] 커뮤니티 유지 예제 편집 팩
