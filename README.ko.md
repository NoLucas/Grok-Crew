...
3. **연결**에서 붙일 글을 복사해 봇 창에 넣습니다. 복사만으로는 연결되지 않습니다. 봇이 `GROK_CREW_OK`만 보내고 멈추면 아직 일이 안 간 겁니다. **시작**에서 나온 글을 그 창에 붙여야 그 자리 일을 합니다. 채팅에 루틴·예약 작업·`keep`을 만들지 않습니다. **연결 해제**는 `disconnected`를 보냅니다.
...
자세한 목록은 [CHANGELOG.md](CHANGELOG.md)와 [1.0.19 릴리스 노트](docs/RELEASE_NOTES.v1.0.19.md)입니다. 손님용 드라이브 파일은 그대로입니다. 이미 설치한 책상은 Grok-crew-test의 v1.0.19를 봅니다.
