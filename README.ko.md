...
- **쇼츠가 힘들었던 사람용입니다.** 타임라인을 처음부터 짜지 않아도 됩니다. 하고 싶은 말을 적고, 봇 창에 붙여 넣고, 여기서 저장합니다.
- **지금 무료입니다.** 로그인 칸이 없습니다. 구독이 아닙니다. 컷당 크레딧도 없습니다.
- **파일은 이 PC에 남습니다.** 클라우드 편집기가 아닙니다.

이 창이 영상을 만들어 주는 구독이 아닙니다. 쓰던 봇을 붙이는 프로그램입니다.
...
| 자리 | 기본 스킬 | 붙는 추가 스킬 |
|---|---|---|
| **Grok Bot / Agent 기획자** | `planner` — 오늘 할 말을 컷 계획으로 | `edit-plan` — 수집·편집자가 읽는 짧은 계획서 |
| **Grok Bot / Agent 스크래핑** | `scraper` — 공개 페이지만 고름 | `public-pick` — 로그인 벽이 있는 SNS는 적지 않음 |
| **Grok Bot / Agent 편집자** | `editor` — 계획대로 자르기 | `cut-to-plan` — 장면 순서와 나라 버릇을 지킴 |

원문은 [`public/bot-skills/`](public/bot-skills/)에 있습니다. 연결 글을 복사하면 그 자리의 스킬이 같이 갑니다. 이 프로그램은 사이트를 긁지 않습니다.
...
예전 Grok-Crew(0.2.1)는 브라우저 작업대에 가깝습니다. 지금 1.0.0은 **이 PC 창**입니다.

- 연결 · 자동 · 설정 · 편집 · 내보내기가 같은 쓰기입니다. 할 일 하나, 나머지는 칩.
- **Grok Bot**과 **Agent**를 기획자·스크래핑·편집자로 붙입니다. 내장 스킬이 같이 갑니다.
- 붙일 글만 복사합니다. 답장을 이 창에 다시 붙이지 않습니다. `127.0.0.1`을 봇에게 보여 주지 않습니다.
- 자막·더빙·TTS는 켠 것만 돌아갑니다. 기본은 모두 끔입니다.
- 다듬은 파일은 이 PC에 남습니다. GitHub 편지함은 없어도 됩니다.
- 짝 코드는 암호용 난수입니다. GitHub 토큰 오류에 토큰을 다시 적지 않습니다.
