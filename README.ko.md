...
3. **연결**에서 붙일 글을 **다시** 복사해 봇 창에 넣습니다. Grok Bot은 등록된 Windows에서 승인 후 체크인하면 붙습니다. 붙인 뒤 그 Windows에서 `keep`을 한 번 돌립니다. 1분마다 자리 확인은 `keep`이 하고, 채팅 예약은 만들지 않습니다. **연결 해제**는 `disconnected` 명령을 보내 `keep`을 멈춥니다. Linux Grok은 `GROK_CREW_OK`만 보내고 멈춥니다. 그 줄을 **연결** 칸에 붙이면 이 책상이 입장합니다. 복사만으로는 연결되지 않습니다.
...
자세한 목록은 [CHANGELOG.md](CHANGELOG.md)와 [1.0.16 릴리스 노트](docs/RELEASE_NOTES.v1.0.16.md)입니다. 손님용 드라이브 파일은 그대로입니다. 이미 설치한 책상은 Grok-crew-test의 v1.0.16을 봅니다.
