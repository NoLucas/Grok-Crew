...
3. **연결**에서 붙일 글을 **다시** 복사해 봇 창에 넣습니다. Grok Bot은 등록된 Windows에서 승인 후 체크인하면 붙습니다. 붙인 뒤 그 Windows에서 `keep`을 한 번 돌립니다. 1분마다 자리 확인은 `keep`이 하고, 채팅 예약은 만들지 않습니다. **연결 해제**는 `disconnected` 명령을 보내 `keep`을 멈춥니다. 안 되면 봇이 보낸 `GROK_CREW_OK` 한 줄을 여기 붙입니다. 복사만으로는 연결되지 않습니다.
