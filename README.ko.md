...
3. **연결**에서 붙일 글을 복사해 봇 창에 넣습니다. 그 글을 복사하면 이 책상이 그 자리로 입장하고 자리 확인을 합니다. 봇의 첫 답은 `GROK_CREW_OK` 한 줄이고, 그다음부터 그 자리 일만 합니다. 채팅에 루틴·예약 작업·`keep`을 만들지 않습니다. **연결 해제**는 `disconnected`를 보냅니다.
