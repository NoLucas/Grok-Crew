...
3. **연결**에서 붙일 글을 복사해 봇 창에 넣습니다. 복사만으로는 연결되지 않습니다. 봇이 `GROK_CREW_OK`를 보낸 뒤, **시작**에서 나온 글을 그 창에 붙이면 그 자리 일을 합니다. 채팅에 루틴·예약 작업·`keep`을 만들지 않습니다. **연결 해제**는 `disconnected`를 보냅니다.
