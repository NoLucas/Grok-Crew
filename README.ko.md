...
3. **연결**에서 붙일 글을 복사해 봇 창에 넣습니다. 복사와 채팅의 「연결됨」은 불이 아닙니다. 봇이 두 줄로 답하면 이 책상이 입장합니다. 불은 이 Windows에 들어온 뒤에만 켜집니다. 이미 붙은 봇은 연결 글을 다시 복사하세요. 채팅에 루틴·예약 작업·`keep`을 만들지 않습니다. **연결 해제**는 `disconnected`를 보냅니다.
4. **시작**에 하고 싶은 말을 적고 **제작 시작**을 누릅니다. 이 Windows에 Grok 자리가 들어와 있으면 그 자리가 `next-invite`를 한 번 읽습니다. 다시 복사는 숨깁니다. 이 창은 기다립니다. **크루 보드** 대화는 자리마다 남긴 시작·넘김 한 줄이 채팅처럼 쌓입니다.
5. 끝난 컷은 **여기에 놓기**에 올라옵니다. **최근기록**에도 같은 파일입니다. **이 PC에 저장**합니다. 올리는 일은 그다음, 원할 때만.
...
- 어제 저장된 OK만으로는 다시 복사를 숨기지 않습니다. 이번 실행·이번 연결 글 뒤에 맞춘 입장만 숨깁니다.
- Agent 불도 이 Windows 명단 입장 뒤에만 켭니다. 클립보드 OK만으로는 꺼져 있습니다.
- 인박스 개수가 그대로여도 이름·시각·크기가 바뀌면 여기에 놓기로 봅니다.
- 수집함·자료함에만 있으면 잘못된 문을 한 줄로 알립니다. 최근기록 실패는 **다시 옮기기**로 한 번 더 시도합니다.
- 연결 글이 옛 버전이면 시작 칸을 연 채로 둡니다. 이미 붙은 봇은 연결 글을 다시 복사하세요.

자세한 목록은 [CHANGELOG.md](CHANGELOG.md)와 [1.0.22 릴리스 노트](docs/RELEASE_NOTES.v1.0.22.md)입니다. 손님용 드라이브 파일은 그대로입니다. 이미 설치한 책상은 Grok-crew-test의 v1.0.22를 봅니다.

## v1.0.21에서 달라진 것

- 어제 leftover 명단만으로는 다시 복사를 숨기지 않습니다. 불이 켜져 있어도 이 자리 글이 안 갔으면 시작 글을 붙입니다.
- 이 대기를 연 뒤의 인박스 파일만 여기에 놓기를 닫습니다. 그보다 오래된 폴더는 고르지 않습니다.
- 최근기록으로 옮기지 못하면 한 줄로 알립니다.

1.0.21 목록은 [1.0.21 릴리스 노트](docs/RELEASE_NOTES.v1.0.21.md)입니다.

## v1.0.20에서 달라진 것

- 불은 이 Windows에 들어온 뒤에만 켜집니다. 채팅의 「연결됨」이나 클립보드 `GROK_CREW_OK`만으로는 불이 아닙니다.
- 이미 붙은 Grok 자리는 시작 이후 `next-invite`를 한 번 읽습니다. 다시 복사는 숨깁니다.
- 이 대기를 연 뒤 새로 온 인박스 파일은 규격 id가 없어도 여기에 놓기를 닫을 수 있습니다. 어제 남은 파일은 닫지 않습니다.
- 끝난 컷의 집은 여기에 놓기입니다. 최근기록에도 같은 파일입니다.

1.0.20 목록은 [1.0.20 릴리스 노트](docs/RELEASE_NOTES.v1.0.20.md)입니다.

## v1.0.19에서 달라진 것

- 연결 글 복사는 기다림입니다. 불은 봇의 짧은 `GROK_CREW_OK`가 온 뒤에만 켜집니다.
- 봇이 `GROK_CREW_OK`만 보내고 멈추면 실패한 시작입니다. 첫 답은 OK 한 줄과 준비됨 한 줄입니다.
- 시작은 연결 글을 쓴 그 창에 일 글을 붙여 넣습니다. 리눅스 Grok은 `127.0.0.1`을 열지 않습니다.
- 왼쪽 영상 칸은 다시 누르거나 `❚❚ 정지`로 멈출 수 있습니다.

1.0.19 목록은 [1.0.19 릴리스 노트](docs/RELEASE_NOTES.v1.0.19.md)입니다.
...
1.0.18 목록은 [1.0.18 릴리스 노트](docs/RELEASE_NOTES.v1.0.18.md)입니다. 그 설치 파일은 복사만으로 불을 켰고, 1.0.19에서 고쳤습니다.
