...
첫 화면은 짧습니다. 제목, 영상 놓기, **봇에게 이 말 복사**. 그 저장은 봇 하나입니다(`source_mode: bot`). Cursor 에이전트는 같은 PC면 체크인하고, 다른 컴퓨터면 초대문의 편집 인박스 폴더에만 둡니다. 스타일 레시피와 봇 둘 규격은 **더 자세히** 뒤에 있습니다. 편집 문으로 온 컷은 받기 버튼 없이 열립니다. 봇이 저장한 폴더(`inputs/handoff/` 패키지와 자료함 클립)는 한 줄로 접혀 있고, 펼치면 목록 옆에 미리보기가 있습니다. 파일을 오른쪽 클릭하면 미리보기·크게 보기·원본 보기·삭제를 고릅니다. 화면 비율·화질·자막은 규격 잠금이고, 나머지 편집 조절은 설정에서 바꿀 수 있습니다. 밝은 낮·어두운 밤·부드러운 낮·부드러운 밤과 글자 강조·크기는 왼쪽 위 톱니바퀴에서 바꿉니다. 이 책상은 사이트를 긁지 않습니다.
