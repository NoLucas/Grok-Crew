...
首屏很短：标题、放视频、**复制给机器人**。这次保存只有一个机器人（`source_mode: bot`）。本机的 Cursor 代理可以签到；远程 Cursor 代理只把文件夹放到邀请里的剪辑收件箱。风格配方和双机器人规格在**更详细**后面。剪辑门送来的成片会自己打开。机器人保存的文件夹默认收成一行，展开后列表在左、预览在旁；右键可预览、放大、打开原片或删除。画质按规格锁定，画面比例、字幕和其余剪辑控件仍可改。晴昼、暗夜、柔昼、柔夜和文字在左上角齿轮里改。这个工作台不抓网站。
