...
3. 在 **连接** 把接上用的文字复制到机器人窗口。复制，或聊天里说「已连接」，都不是灯。两行回复后这张书桌会签到。灯只在进入这台 Windows 后亮。已经接上的机器人请再复制一次连接稿。不要在聊天里做 Routine、预约作业或 `keep`。**断开连接**会发送 `disconnected`。
4. 在 **开始** 写下想做的事，按 **开始制作**。已在这台 Windows 上的 Grok 位子会读一次 `next-invite`。再复制会藏起来。这个窗口等待。**组员看板**对话按位子把开工和转交的一行堆成聊天。
5. 完成的成片出现在 **放在这里**。**最近记录**是同一份文件。然后 **存到这台电脑**。发布是下一步，而且只有他们想发时才做。
...
- 昨天留下的 OK 不会藏起再复制。只有这次运行、这次连接稿之后的签到才藏。
- Agent 灯也要等这台 Windows 名册签到。剪贴板只有 OK 时是灭的。
- 收件箱个数没变，只要名字、时间或大小变了，「放在这里」也会看。
- 文件只在收集箱或资料箱时会说一句门错了。最近记录失败可用「再移一次」。
- 旧版连接稿会把开始栏留着。已经接上的机器人请再复制一次。

完整列表见 [CHANGELOG.md](CHANGELOG.md) 和 [1.0.22 发布说明](docs/RELEASE_NOTES.v1.0.22.md)。客人用的网盘文件不变。已安装的书桌看 Grok-crew-test 的 v1.0.22。
...
1.0.21 列表见 [1.0.21 发布说明](docs/RELEASE_NOTES.v1.0.21.md)。

## v1.0.20 改了什么

- 灯只在进入这台 Windows 后亮。聊天里说已连接，或剪贴板只有 `GROK_CREW_OK`，都不是灯。
- 已接上的 Grok 位子在开始之后读一次 `next-invite`。再复制会藏起来。
...
1.0.19 列表见 [1.0.19 发布说明](docs/RELEASE_NOTES.v1.0.19.md)。
