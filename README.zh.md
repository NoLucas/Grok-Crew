...
3. **连接** — 打开上面的 **连接** 菜单。另一台电脑分成 Grok Bot 栏和 Agent 栏。在策划、抓取、剪辑上点 **复制连接文字** 就算接上。把那篇贴到机器人窗口。
4. **写下** — 在 **自动** 写一栏就够。画面、去处、声音需要时再开。字幕、我的声音、做声音想开再开。打开做声音才选性别、感觉、口音，并可保存。
...
首屏是 **连接** 菜单。另一台电脑在最上面。这台电脑、Local Studio、GitHub、Grok 制作器 也在那里，默认收起。绿灯就是 **已连接**。接上机器人或自己打开视频后，会落到 **自动**。设置、编辑、导出这时才打开。每个标签先做一件事，并用摘要芯片收起其余选项。**自动**是今天要发的话、什么样的视频、资料（你放入的视频或图片 / 抓取机器人要取的东西）和一次开始。这个应用不抓站。交给它、用自己的文件是自动里的两种方式。连接界面不在自动里再画一遍。示例只通过**用示例查看画面**打开。复制之后五盏灯显示连接、已发送、工作中、成片到达、留在这台电脑，以及经过时间、还没有 / 已到达 / 失败。这个窗口不声称机器人已读邀请。保存卡片写出这台电脑的文件夹，并说明自动没有发布。python、端口、文件夹路径默认收起。**更详细**和**高级工具**只在第一段成片到达后变大。剪辑门送来的成片会自己打开，预览留在自动标签。机器人保存的文件夹默认收成一行，展开后列表在左、预览在旁；右键可预览、放大、打开原片或删除。画质按规格锁定，画面比例、字幕和其余剪辑控件仍可改。设置里可选 Shorts、Reels、TikTok 等风格，或把当前选项存成名字。晴昼、暗夜、柔昼、柔夜和文字在左上角齿轮里改。这个工作台不抓网站。

然后是五个标签：

| 标签 | 用来做什么 |
| --- | --- |
| **连接** | 另一台电脑（Grok Bot / Agent × 策划、抓取、剪辑）、这台电脑、Local Studio、GitHub、Grok 制作器 |
| **自动** | 今天的话 → 发送 → 等待 → 预览 → 保存 |
| **编辑** | 看预览，在时间线上裁切 |
| **设置** | 风格、字幕、速度 |
