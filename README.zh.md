...
4. **写下** — 在 **自动** 写下今天要发的话。字幕、配音、TTS 想开再开。打开 TTS 才选一个声音。只按下一步就是 Kokoro-82M。
