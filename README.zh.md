...
3. 在 **连接** 把接上用的文字**再**复制到机器人窗口。Grok Bot 应在已登记的 Windows 上批准后签到。贴上后在那台 Windows 上把 `keep` 跑一次。每分钟位子确认是 `keep` 做的，不是聊天预约作业。**断开连接**会发送 `disconnected`，`keep` 看到后停下。没到的话，把机器人回的 `GROK_CREW_OK` 贴回这里。只复制不算已连接。
4. 在 **开始** 写下想做的事，按 **开始制作**。同一台 Windows 上已签到的 Grok 位子会自己读待办邀请。这个窗口等待。**组员看板**显示谁做了什么，以及留给下一位子的一行。
