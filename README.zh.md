...
3. **连接** — 打开上面的 **机器人** 菜单。这台电脑贴签到文字。另一台电脑（Grok 或自己的智能体）先贴连接文字，再把它们回的一行 `GROK_CREW_OK` 贴回来。出现名字就是已连接。
4. **写下** — 标题。
...
另一台电脑上的 AI 打不开这张桌子。**机器人** 菜单只按名字接上，成片再放到这个窗口。不会把素材寄出去让别处的机器人「看一眼」。
