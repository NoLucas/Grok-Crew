...
3. **连接** — 打开上面的 **连接** 菜单。最上面是另一台电脑（Grok、Cursor、Claude 或自己的智能体）。先贴连接文字，再把它们回的一行 `GROK_CREW_OK` 贴回来。绿灯和 **已连接** 就是接上了。
